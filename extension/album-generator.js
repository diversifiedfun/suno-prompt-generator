...
Write full, specific, non-cliché lyrics on the track's angle/subject using [Section] tags; if a MOMENTARY production/FX effect helps a line, WRITE the inline tag ([Vocal Chop], [Harmonies], etc.) directly into the lyrics — the field must be paste-ready. VOCAL DELIVERY MODES (whispered, belted, falsetto, spoken-word, breathy, soft/intimate): Suno ignores a mid-line "(whispered)"/"(belt)" cue and a bare inline [Belt]. Control delivery TWO ways that must AGREE: (a) state the mode in the STYLE field (e.g. "intimate breathy verses, belted chorus payoff"), and (b) tag the SECTION with a SHORT bracket on its own header line — [Chorus] [Belting, Powerful], [Verse] [Soft, Intimate], [Bridge] [Whispered, Intimate] — then clean lyric lines beneath (never put lyric words inside brackets). Use the tested tag forms ([Belting] not [Belt], [Whispered] not [Whisper], [Spoken Word] not [Spoken]); keep each tag 1–3 words. Put contrast BETWEEN sections (whispered verse → belted chorus), never contradictory words in one tag. A true whisper also needs that section kept sparse — Suno won't whisper over a wall of sound. LYRIC CRAFT: keep lines even (each within ±2 syllables of the others, or Suno rushes/glitches); favor ABAB + slant / near rhymes over sing-song AABB perfect rhymes; keep the chorus short and repeat its exact text; respell tricky words, NAMES, and numbers phonetically (Evie→Ee-vee, 2am→three A-M). Never emit numeric/parameter tags like [Reverb: 30%] (proven placebo) — use words (reverb-drenched). Never emit artist names, empty evaluative words, or medical claims.
